            ...
            for(index=0;index<PORT_CONFIGURED_PINS;index++){
               ...
               /* Point to the correct PORT register according to the Port Id stored in the Port_Num member */
                switch(Port_PortPins[index].port_num){
                    case PORT_PortA:Port_base=GPIO_PORTA_BASE_ADDRESS;
                               break;
                    case PORT_PortB:Port_base=GPIO_PORTB_BASE_ADDRESS;
                               break;
                    case PORT_PortC:Port_base=GPIO_PORTC_BASE_ADDRESS;
                               break;
                    case PORT_PortD:Port_base=GPIO_PORTD_BASE_ADDRESS;
                               break;
                    case PORT_PortE:Port_base=GPIO_PORTE_BASE_ADDRESS;
                               break;
                    case PORT_PortF:Port_base=GPIO_PORTF_BASE_ADDRESS;
                               break;
                }
                ...
                if( (Port_PortPins[index].port_num==PORT_PortF && Port_PortPins[index].pin_num==PORT_Pin0)\
                  ||(Port_PortPins[index].port_num==PORT_PortD && Port_PortPins[index].pin_num==PORT_Pin7)){
                    /* Unlock the GPIOCR register */
                    *((volatile uint32 *)(Port_base+PORT_LOCK_REG_OFFSET))    = UNLOCK_VALUE;
                ...
                else if((Port_PortPins[index].port_num == PORT_PortC) && (Port_PortPins[index].pin_num <= PORT_Pin3 )){
                ...
                if(Port_PortPins[index].pin_mode==PORT_PIN_MODE_DIO){
                ...
                else if(Port_PortPins[index].pin_mode==PORT_PIN_MODE_ADC){
...
/* Check that development error is on */
       ...
       if(error==FALSE){
           ...
           if((Port_PortPins[Pin].port_num == PORT_PortC) && (Port_PortPins[Pin].pin_num <= PORT_Pin3 )){
               /* JTAG PINS FOR DEBUGGING ONLY */
               return;
           }
           else{
       ...
       if(error==FALSE){
           ...
           for(index=0;index<PORT_CONFIGURED_PINS;index++){
               ...
               if(Port_PortPins[index].direction_changeable==STD_ON){
                   ...
                   if((Port_PortPins[index].port_num == PORT_PortC) && (Port_PortPins[index].pin_num <= PORT_Pin3 )){
                       /* JTAG PINS FOR DEBUGGING ONLY */
                       continue;
                   }
                   else{
